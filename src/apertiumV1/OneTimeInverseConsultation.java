...
public class OneTimeInverseConsultation {
	...
	public  ArrayList<TranslatablePair> obtainTranslationScoresFromLemma(String sourceLemma, String sourceLanguage, String pivotLanguage, String targetLanguage){
		...
		if(sourceLanguage == "en")
			sourceLexicon = "http://linguistic.linkeddata.es/id/apertium/lexiconEN";
		else
			sourceLexicon = SPARQLSearches.obtainLexiconFromLanguage(sourceLanguage);			
		// Get Lexical entries associated to source label
		ArrayList<String> sourceLexicalEntries = SPARQLSearches.obtainLexicalEntriesFromLemma(sourceLexicon, sourceLemma, sourceLanguage);		
	...
	public  ArrayList<TranslatablePair> obtainTranslationScoresFromLexicalEntry(String sourceLemma, String sourceLexicalEntry, String translationSetURI1, String translationSetURI2){
		...
		ArrayList<String> pivotTranslations = SPARQLSearches.obtainDirectTranslationsFromLexicalEntry(translationSetURI1, sourceLexicalEntry.toString());		
		...
		for (String pivotTranslation: pivotTranslations){			
			ArrayList<String> newTargetTranslations= SPARQLSearches.obtainDirectTranslationsFromLexicalEntry(translationSetURI2, pivotTranslation.toString());
			...
			for (String targetTranslation: newTargetTranslations){				
				...
				String queryString = 
					...
					" WHERE { " +
					" <" + targetTranslation.toString() + "> lemon:lexicalForm ?form ;" + 
					"          lexinfo:partOfSpeech ?pos." +
					"  ?form lemon:writtenRep ?lemma ." +
					"}";
				Query query = QueryFactory.create(queryString);
				QueryExecution qe = QueryExecutionFactory.sparqlService(SPARQLSearches.getSparqlEndpoint(), query);
		...
		for (TranslatablePair ts:translatablePairs){	
				...
				P2.addAll(SPARQLSearches.obtainDirectTranslationsFromLexicalEntry(translationSetURI2, ts.getTargetLexicalEntry().toString()));
